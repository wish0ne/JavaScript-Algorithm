...
function solution(jobs) {
  ...
  while (i < jobs.length || !heap.isEmpty()) {
    //현재 time보다 요청타임이 이하인것들 모두 heap에 넣음
    if (i < jobs.length) {
      if (jobs[i][0] <= time) {
        heap.add([jobs[i][1], jobs[i][0]]);
        i += 1;
        continue;
      }
    }

    //현재 time에서 실행가능한 것들을 모두 heap에 넣음
    //실행할게 있으면 실행
    if (!heap.isEmpty()) {
      const [duration, start] = heap.poll();
      arr.push(time + duration - start);
      time += duration;
    }
    //실행할게 없다면 time 1 증가
    else time += 1;
  ...
  //평균 소요시간 계산
  console.log(arr);
  return parseInt(arr.reduce((prev, curr) => prev + curr) / arr.length);
}
