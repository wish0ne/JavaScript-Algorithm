...
- 2차원 배열 사용
- 간선 정보 저장하기 위해 $O(V^2)$의 메모리 공간 필요(노드 개수 : V, 간선 개수 : E)
- 특정 노드 A에서 B로 이어진 간선 비용 찾을때 $O(1)$소요
- 플로이드 워셜은 인접행렬 이용하는 방식

#### 2. 인접리스트

- 리스트 사용
- 간선 정보 저장하기 위해 $O(E)$만큼의 메모리 공간 필요
- 특정 노드 A에서 B로 이어진 간선 비용 찾을때 $O(V)$소요
- 다익스트라는 인접리스트 사용하는 방식

> 노드의 개수가 적으면 플로이드 워셜 사용, 노드와 간선의 개수가 모두 많으면 다익스트라 사용하면 유리
...
- **union, find** 2개의 연산으로 조작가능
- **union** : 2개의 원소가 포함된 집합을 하나의 집합으로 합치는 연산
- **find** : 특정한 원소가 속한 집합이 어떤 집합인지 알려주는 연산
- 서로소 집합 자료구조를 구현할때는 **트리 자료구조** 이용

1. union 연산을 확인하여, 서로 연결된 두 노드 A, B를 확인
   1. A와 B의 루트노드 A', B'을 찾음
   2. A'를 B'의 부모로 설정(B'가 A'를 가리키도록함)
2. 모든 union 연산을 처리할때까지 1번 과정을 반복

- 노드 개수가 V, 최대 V-1개의 union연산과 M개의 find연산이 가능할때 경로압축방법을 적용한 시간복잡도는 O(V+M(1+log<sub>2-M/V</sub>V))

### 서로소 집합을 이용한 사이클 판별

- 서로소 집합은 **무방향 그래프 내**에서의 사이클 판별에 사용가능
- 간선을 하나씩 확인하면서 두 노드가 포함되어 있는 집합을 합치는 과정을 반복하는것으로 사이클 판별 가능
- 방향 그래프에서의 사이클 판별은 DFS 이용
...
> 신장 트리 중에서 최소비용으로 만들 수 있는 신장 트리를 찾는 알고리즘

- 크루스칼 알고리즘이 있음
- 크루스칼 알고리즘은 그리디로 분류
  - 가장 거리가 짧은 간선부터 집합에 포함시킴
  - 사이클을 발생시킬 수 있는 간선의 경우 집합에 포함시키지 않음
- 최소신장트리는 트리 자료구조이므로 간선의 개수가 **노드개수-1**
- 간선이 E개일때 O(ElogE)의 복잡도
  - 간선을 정렬하는 작업이 가장 오래걸리는 부분. E개의 데이터를 정렬할때 시간복잡도가 O(ElogE)
...
- 모든 원소를 방문하기 전에 큐가 빈다면 사이클이 존재
  - 원소가 V번 추출되기 전에 큐가 비어버리면 사이클이 존재
  - 사이클이 존재하는경우 사이클에 포함되어 있는 원소중에서 어떠한 원소도 큐에 들어가지 못하기 때문
- 위상 정렬의 답은 여러가지가 될 수 있음
- 시간복잡도는 O(V+E)
  - 모든 노드를 차례대로 확인하면서, 해당 노드에서 출발하는 간선을 차례대로 제거 (모든 노드와 간선을 확인)
...
> 한 번의 시행에서 <strong>일어날 수 있는 사건의 가지 수</strong>

### 순열

> 서로 다른 n개에서 r개를 선택하여 일렬로 나열하는 것 : $_nP_r = \frac{n!}{(n-r)!}$

코테에서는 경우의 수 값만 출력하는것 뿐만 아니라 모든 경우(사건)을 다 출력하도록 요구하기도함.

### 조합

> 서로 다른 n개에서 순서에 상관없이 서로 다른 r개를 선택하는 것 : $_nC_r = \frac{n!}{r!\times(n-r)!}$
